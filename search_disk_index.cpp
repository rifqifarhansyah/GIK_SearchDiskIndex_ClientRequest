...
int main(int argc, char **argv)
{
    ...
    uint32_t num_threads, K, W, num_nodes_to_cache, search_io_limit, number_of_request;
    ...
    try
    {
        ...
        optional_configs.add_options()("number_of_request,N", po::value<uint32_t>(&number_of_request)->default_value(5));
    ...
    std::vector<double> latencies(number_of_request, 0.0);
    std::vector<double> qps_values(number_of_request, 0.0);
    ...
    try
    {
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("float"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<float, uint16_t>(metric, index_path_prefix, result_path_prefix,
                                                                     query_file, gt_file, num_threads, K, W,
                                                                     num_nodes_to_cache, search_io_limit, Lvec,
                                                                     fail_if_recall_below, query_filters,
                                                                     use_reorder_data);
                        if (ret != 0)
                        {
                            diskann::cerr << "Index search failed on request " << req_id << std::endl;
                            std::exit(-1);
                        }
                        auto end_time = std::chrono::high_resolution_clock::now();
                        std::chrono::duration<double> elapsed_seconds = end_time - start_time;

                        // Calculate latency and QPS for this request
                        latencies[req_id] = elapsed_seconds.count();
                    });
                }
            ...
            else if (data_type == std::string("int8"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<int8_t, uint16_t>(metric, index_path_prefix, result_path_prefix,
                                                                      query_file, gt_file, num_threads, K, W,
                                                                      num_nodes_to_cache, search_io_limit, Lvec,
                                                                      fail_if_recall_below, query_filters,
                                                                      use_reorder_data);
            ...
            else if (data_type == std::string("uint8"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<uint8_t, uint16_t>(metric, index_path_prefix, result_path_prefix,
                                                                       query_file, gt_file, num_threads, K, W,
                                                                       num_nodes_to_cache, search_io_limit, Lvec,
                                                                       fail_if_recall_below, query_filters,
                                                                       use_reorder_data);
        ...
        else
        {
            if (data_type == std::string("float"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<float>(metric, index_path_prefix, result_path_prefix, query_file,
                                                            gt_file, num_threads, K, W, num_nodes_to_cache,
                                                            search_io_limit, Lvec, fail_if_recall_below,
            ...
            else if (data_type == std::string("int8"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<int8_t>(metric, index_path_prefix, result_path_prefix, query_file,
                                                             gt_file, num_threads, K, W, num_nodes_to_cache,
                                                             search_io_limit, Lvec, fail_if_recall_below,
                                                             query_filters, use_reorder_data);
                        if (ret != 0)
            ...
            else if (data_type == std::string("uint8"))
            {
                ...
                for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
                {
                    threads.emplace_back([&]() {
                        auto start_time = std::chrono::high_resolution_clock::now();
                        int ret = search_disk_index<uint8_t>(metric, index_path_prefix, result_path_prefix, query_file,
                                                              gt_file, num_threads, K, W, num_nodes_to_cache,
                                                              search_io_limit, Lvec, fail_if_recall_below,
                                                              query_filters, use_reorder_data);
                        if (ret != 0)
        ...
        // Calculate the average latency and QPS across all requests
        double total_latency = 0.0;
        double total_qps = 0.0;
        for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
        {
            total_latency += latencies[req_id];
            total_qps += qps_values[req_id];
        }
        double avg_latency = total_latency / number_of_request;
        double avg_qps = total_qps / number_of_request;

        // Print the results
        std::cout << "Latencies for each request:" << std::endl;
        for (uint32_t req_id = 0; req_id < number_of_request; req_id++)
        {
            std::cout << "Request " << req_id + 1 << ": " << latencies[req_id] << " seconds" << std::endl;
        }
